...
func (swarmStack *swarmStack) updateStack(currentStatus *StackStatus) (stackMetadata *stackMetadata, err error) {
	...
	deployedMetadata, err := loadLastDeployedMetadata(swarmStack.name)
	if err != nil {
	...
	if swarmStack.shouldDeploy(updatedMetadata, deployedMetadata) {
		...
		err = saveLastDeployedMetadata(swarmStack.name, updatedMetadata)
		if err != nil {
		...
		err = saveLastDeployedMetadata(swarmStack.name, deployedMetadata)
		if err != nil {
